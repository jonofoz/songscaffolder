...
def config(request, field_name):
    ...
    field_data = request.session["user_data"].get(field_name.replace("-", "_"), {"Sample Field": "Enter Your Frequency Here! (1-5)"})
    context = {
...
def delete(request, data_id):
    ...
    try:
        if len(data_id_split) == 2:
            del request.session["user_data"][data_id_split[0]][data_id_split[1]]
        elif len(data_id_split) == 3:
            del request.session["user_data"][data_id_split[0]][data_id_split[1]][data_id_split[2]]
        elif len(data_id_split) == 4:
            del request.session["user_data"][data_id_split[0]][data_id_split[1]][data_id_split[2]][data_id_split[3]]
        request.session.save()
    except:
        pass
    finally:
        return redirect('config', data_id_split[0])